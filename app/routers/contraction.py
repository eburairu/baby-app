...
@router.post("/{contraction_id}/end", response_model=None)
...
async def create_contraction(
    ...
    # HTML レスポンスは未実装
    raise HTTPException(status_code=400, detail="HTML response not supported for manual creation")


@router.put("/{contraction_id}", response_model=None)
async def update_contraction_json(
    contraction_id: int,
...
@router.delete("/{contraction_id}", response_model=None)
